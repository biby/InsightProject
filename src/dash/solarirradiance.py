...
def dayTime(date, latitude):
    ...
    nbDaysInYear= nbDaysSinceBeginingOfYear(date)
    earthtilt = earthTilt(date)
    x = - (sin(degToRad(latitude)) * sin(degToRad(earthtilt)))
    x = x / (cos(degToRad(latitude)) * cos(degToRad(earthtilt)))
...
def  currentRadiation( date,  latitude,  hour, moduleAngle):
    sr = sunrise(date, latitude)
    ss = sunset(date, latitude)
    if hour < sr:
        return 0.0
    if hour > ss:
        return 0.0
    hourfactor = sin((hour-sr)*pi/dayTime(date,latitude)) #Use a sinusoid for the the day variation. May be improved.
    """am = AM(hour, day, lat)
    x1 = 0.7**(am*0.678)""" 
